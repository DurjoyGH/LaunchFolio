export const buildBlueprintPrompt = (userInput: any, registry: any) => {
  const {
    name,
    title,
    bio,
  ...
  const skillNames = skills.map((s: any) => s.name).join(", ") || "None";
  const projectTitles = projects.map((p: any) => p.title).join(", ") || "None";
  const serviceTitles = services.map((s: any) => s.title).join(", ") || "None";
...
If user has no projects, you may skip "projects".`;
  ...
  if (registry) {
    for (const [section, variants] of Object.entries(registry)) {
      availableVariants += `- ${section}: ${shuffle(variants as any[]).join(", ")}\n`;
    }
  } else {
...
export const buildContentPrompt = (userInput: any, blueprint: any) => {
  return `
